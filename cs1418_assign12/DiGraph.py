...
Program description:DIRECTED GRAPH IN PYTHON
Acknowledgements:
--------------------------------------------------------------------'''
...
#initialize class
    ...
    def has_edge(self, v1, v2):# detect if it has edge
        return self.G.has_key(v1) == True and v2 in self.G.get(v1)

    def has_vertex(self, v):# detect if it has vertex
    ...
    def add_vertex(self, v=None):# add a vertex
        if v != None:
            if v in self.G.keys():
                return 1
            self.G[v] = []
    ...
    def add_vertices(self):# add vertices
    ...
    def add_edge( # add edge
        ...
        if v1 != None and v2 != None and w != None:
    ...
    def delete_edge(self, v1=None, v2=None):# delete an edge
        if v1 == None or v2 == None:
            return 1
        elif (self.G.has_key(v1) == False) or (v2 in self.G[v1] == False):
            return 1
        else:
            k = self.G.get(v1)
            m = self.W.get(v1)
    ...
    def delete_edges(self):# delete multiple edges
        print 'Enter the no of edges'
        E = raw_input()
        for rowindex in range(0, int(E)):
            print 'Enter the vetex of origin of the edge'
            ...
            if v1 != None and v2 != None:
                if (self.G.has_key(v1) == False) or (v2 in self.G[v1] \
                    == False):
    ...
    def indeg_vertex(self, v): # indegree
        count = 0
    ...
    def degree(self, v):# degree returns indegree and outdegree as tuple
        if v in self.G.keys():
            return (self.indeg_vertex(v), self.outdeg_vertex(v))
        else:
            print 'vertex not in Graph'
            return None

    def outdeg_vertex(self, v):# out degree
        return len(self.G[v])

    def adjacency_matrix(self): # convert to adjacency matrix
        count = 0
        AdjMat = []
        Map = {}
    ...
    def induced_subgraph(self, *ver):# get teh induced subgraph
        l = {}
        w = {}
 ...
 #DFS (to visit a vertex v)
    ...
    def depth_first_search(self, v): # main function for depth first search
        Marked = {}
        dfs_traverse = []
...
#Repeat until the queue is empty:
 ...
 # - add each of v's unvisited neighbors to the queue,
  #  and mark them as visited.
    def breadth_first_search(self, v):# breadth first search
        Marked = {}
        q = Queue.Queue()
        bfs_traverse = [v]
        q.put(v)
...
# utility function for shortest path
    def min_distance(# get the minimum distance
        self,
        dist,
        visited,
        Map,
...
# auxillary function to return weight
    def get_weight(self, u, v):# auxillary function o get weight
        if v in self.G[u]:
            i = self.G[u].index(v)
...
#**********************************************************
    def shortest_path(self, u, v): # get shortest path
        dist = [0]
        visited = []
        Map = {}
        index = 0
...
# get all path between two vertices
    def all_path(self, u, v):# get all path
        allpath = {}
        S = []
        index = 0
        Map = {}
...
# find path using depth first search
    def findpath(# find path between two vertices
        self,
        x,
        v,
        Marked,
...
#8 return A
#9  end function
    def mst(self):# minimum spanning tree, kruskal's algorithm
        l = []
        A = []
        Map = {}
        count = 0
...
# get the transpose of a graph
    def transpose(self):
        ...
        for u in self.G.keys():
            for v in self.G[u]:
                if l.has_key(v) == False:
                    l[v] = [u]
                else:
                    l[v].append(u)
                if w.has_key(v) == False:
                    w[v] = [self.get_weight(u, v)]
                else:
                    w[v].append(self.get_weight(u, v))
            if l.has_key(u) == False:
                l[u] = []
                w[u] = []
        return (l, w)
...
#Phase2.  Run DFS in G, visiting unmarked vertices in reverse postorder of G
    def connected_component(self, v=None):# connceted component kosaraju's algorithm
        (l, w) = self.transpose()
        Marked = {}
        dfs_traverse = []
        for i in l.keys():
        ...
        for i in l.keys():
            if Marked[i] == False:
                self.depth_first_order(l, i, Marked, dfs_traverse)
        for i in self.G.keys():
            Marked[i] = False
        count = 0
        id = {}
        while len(dfs_traverse) > 0:
            i = dfs_traverse.pop()
            if Marked[i] == False:
...
# auxillary function for connected components
    def depth_first_order( # utility for connected component
        self,
        l,
        x,
        Marked,
    ...
    def is_connected(self):# check if strongly connected
        Strongly_Connected = self.connected_component()
        id = 1
        for w in self.G.keys():
            if (Strongly_Connected[w] and id) == 0:
                return False
            else:
                id = Strongly_Connected[w]
...
if int(choice) == 1:
    ...
    exit(1)
while True:
    print '*******Directed Graph Utilities*************'
    print '1. Has vertex'
    print '2. Has Edge'
    print '3. Add vertex'
